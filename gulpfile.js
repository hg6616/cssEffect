...
gulp.task('load', ['clean-dist'], function () {
    // return;
    browserSync({
        server: {
            baseDir: distPath,
...
gulp.task('load2', [], function () {
    ...
    return gulp.src(devFilePath.css)
        .pipe(watch(devPath+'**/*.*'))
        .pipe(reload({
            stream: true
        }));
...
//dev
gulp.task('dev', runSequence('load', 'jshint', ['stylus', 'minifycss', 'minifyjs', 'minifyhtml', 'moveImage']));
gulp.task('build', runSequence('load', 'jshint', ['stylus', 'minifycss', 'minifyjs', 'minifyhtml', 'moveImage']));
